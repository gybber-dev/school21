...
{
    try{
        console.log("check_nearest_enemy for " + my_building.id);
        console.log(my_building.id);
        console.log(enemy_buildings);
        ...
        if (dist && dist < CoreObj.criticalDistance)
        {
            ...
            console.log("Army was sent to enemy..........");
            return true;
        }
        return false;
    }catch (e) {
...
function single_attack(my_building, enemy_buildings, neutral_buildings)
{
    try{
        ...
        console.log(game_map.towers_distance(towers[0].id, my_building.id))
        console.log(game_map.towers_distance(towers[1].id, my_building.id))
        console.log(game_map.towers_distance(towers[2].id, my_building.id))
...
{
    try{
        console.log('group_attack');
        my_buildings.forEach(my_building => {
            // если здание простаивает
            if ( ! CoreObj.my_builds[my_building.id] )
            {
                CoreObj.groups = group_towers(my_building);
            }
        });
        if (CoreObj.groups.length)
        {
            // атака группой
        }else{
...
function random_attack(my_buildings, enemy_buildings, neutral_buildings)
{
    try{
        ...
        console.log(enemy_buildings.length);
        console.log(neutral_buildings.length);
...
function apply_abilities(state)
{
    try{
        ...
        if (state.ability_ready(AbilityType[6])) {
            let max_count = 0;
            let min_count = 100;
            let enemy_id = enemy_buildings[0];
            let my_id = my_buildings[0];
            enemy_buildings.forEach(enemy_building => {
                if (enemy_building.creeps_count > max_count && !CoreObj.aims[enemy_building.id])
                {
                    max_count = enemy_building.creeps_count;
