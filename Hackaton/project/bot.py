...
logging.debug('my logs:')

def plan_attack(from_build, to_build, woom):
    distance = game_map.towers_distance(from_build.id, to_build.id)
    ...
    enemy_creeps = 0
    if to_build.creeps_count >= to_build.level.player_max_count:
        # если текущее количество крипов больше чем положено по уровню
        enemy_creeps = to_build.creeps_count
    else:
        # если меньше - будет прирост
        grow_creeps = ticks / to_build.level.creep_creation_time
        enemy_creeps = to_build.creeps_count + grow_creeps
        if enemy_creeps >= to_build.level.player_max_count:
            enemy_creeps = to_build.level.player_max_count
    # определяем количество крипов с учетом бонуса защиты
    enemy_defence = enemy_creeps * (1 + to_build.DefenseBonus)
    # если получается в моей башне крипов больше + 10 на червя - идем на врага всей толпой
    if enemy_defence + woom < from_build.creeps_count:
        return True
    return False
...
while True:
    try:
        ...
        # logging.debug(neutral_buildings)

        # jsonStr = json.dumps(laptop1.__dict__)
        # neutral_buildings.sort(key=lambda c: c.way.left, reverse=False)
        # logging.debug(dir(neutral_buildings))
        # сортируем по остаточному пути
        # my_squads.sort(key=lambda c: c.way.left, reverse=False)


        my_tower = my_buildings[0]
        # сортируем относительно своей башни
        nearest_neurals = list(sorted(neutral_buildings, key=lambda x: game_map.towers_distance(my_tower.id, x.id)))
        
        distance = game_map.towers_distance(my_tower.id, nearest_neurals[0].id)
        # logging.debug("dist to nearest: " + distance)
        # is_attack = plan_attack(my_tower, nearest_neurals[0], 0)
        # print(game_teams.my_her.move(my_tower, nearest_neurals[0].id, 1))
        # logging.debug("attack  to " + nearest_neurals[0].id + "? "+ 'is_attack')
